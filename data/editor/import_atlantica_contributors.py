#!/usr/bin/env python3
"""
Batch import Atlantica contributors.
Creates person nodes for each contributor and links them to Atlantica.
...
CONTRIBUTORS_FILE = Path("/Users/veritas44/Downloads/Caroccio/atlantica_contributors_list.md")
...
def main():
    ...
    try:
        # Get or create Atlantica
        atlantica_id, atlantica_created = get_or_create_node(conn, "Atlantica", "institution")
        if atlantica_created:
            print("Created 'Atlantica' node")
        else:
            print(f"Found existing 'Atlantica' node (id={atlantica_id})")
        ...
        for contributor in contributors:
            ...
            if edge_exists(conn, contributor_id, atlantica_id):
                stats['edges_existed'] += 1
            else:
                create_edge(conn, contributor_id, atlantica_id, 'affiliation')
